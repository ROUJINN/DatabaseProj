...
def student_dashboard():
    ...
    try:
        with conn.cursor() as cursor:
            ...
            # ✅ NEW: Get merchant list from Terminals Table
            cursor.execute(
                "SELECT DISTINCT merchant_name FROM Terminals ORDER BY merchant_name"
            )
            merchants = [row["merchant_name"] for row in cursor.fetchall()]
...
def student_transaction():
    ...
    try:
        with conn.cursor() as cursor:
            ...
            # 获取当前余额
            cursor.execute(
                "SELECT balance FROM Cards WHERE card_id=%s", (card["card_id"],)
            )
            new_balance = cursor.fetchone()["balance"]

            flash(
                f"Transaction successful! 今日累计消费 ¥{today_total:.2f}，当前余额 ¥{new_balance:.2f}"
            )
...
def student_all_transactions():
    ...
    try:
        with conn.cursor() as cursor:
            ...
            return render_template(
                "student_all_transactions.html", transactions=transactions
            )
    finally:
        conn.close()
...
def faculty_all_transactions():
    ...
    try:
        with conn.cursor() as cursor:
            ...
            return render_template(
                "faculty_all_transactions.html", transactions=transactions
            )
    finally:
        conn.close()
            ...
            if request.method == "POST":
                ...
                if action == "delete":
                    ...
                    if card_ids:
                        ...
                        cursor.execute(
                            f"DELETE FROM AccessLogs WHERE card_id IN ({format_strings})",
                            tuple(card_ids),
                        )
                    ...
                    if role == "student":
                        cursor.execute(
                            "DELETE FROM AccessRights WHERE role_value IN (SELECT student_id FROM Students WHERE user_id=%s)",
                            (user_id,),
                        )
                    elif role == "faculty":
                        cursor.execute(
                            "DELETE FROM AccessRights WHERE role_value IN (SELECT staff_id FROM Faculty WHERE user_id=%s)",
                            (user_id,),
                        )
    ...
    try:
        with conn.cursor() as cursor:
            if request.method == "POST":
                ...
                if action == "approve":
                    cursor.execute(
                        "SELECT card_id, new_status FROM CardStatusRequests WHERE request_id=%s",
                        (request_id,),
                    )
                    req = cursor.fetchone()
                    # 更新卡片状态
        ...
        with conn.cursor() as cursor:
            if request.method == "POST":
                ...
                if action == "add":
                    ...
                    # 先检查是否存在
                    cursor.execute(
                        "SELECT 1 FROM AccessPoints WHERE point_id=%s", (pid,)
                    )
                    if cursor.fetchone():
